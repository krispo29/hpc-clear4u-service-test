...
type Repository interface {
	...
	GetAllSeaWaybillDetails(ctx context.Context) ([]*SeaWaybillDetailResponse, error)
...
func (r repository) GetAllSeaWaybillDetails(ctx context.Context) ([]*SeaWaybillDetailResponse, error) {
	db := ctx.Value("postgreSQLConn").(*pg.DB)
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	...
	type row struct {
		...
		DutyTax         float64
		AttachmentsText string `pg:"attachments"`
		CreatedAt       string
		UpdatedAt       string
	}

	var rows []row
	_, err := db.QueryContext(ctx, &rows, sqlStr)
	if err != nil {
		return nil, utils.PostgresErrorTransform(err)
	}
	...
	for _, r := range rows {
		...
		if r.AttachmentsText != "" && r.AttachmentsText != "[]" {
			if err := json.Unmarshal([]byte(r.AttachmentsText), &attachments); err != nil {
				return nil, fmt.Errorf("failed to parse attachments: %w", err)
			}
		}

		results = append(results, &SeaWaybillDetailResponse{
			UUID:         r.UUID,
